...
export class BiddingService {
  ...
  async list(params: {
    ...
    return rows.map((b) => this.toSummary(b, contentByBid.get(b.id), (attachmentCounts.get(b.id) ?? 0) > 0));
  }
  ...
  async getDetail(id: number) {
    ...
    return {
      ...this.toSummary(bid, content, attachments.length > 0),
      jobId: bid.jobId,
  ...
  private toSummary(bid: Bid, content?: BidContent | null, hasAttachments = false) {
